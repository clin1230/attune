...
if req.get('source'):
    ...
    ceramic=material('ceramic_matte_warm_01',(.72,.64,.5),0,.7)
    textile=material('charcoal_textile_01',(.035,.042,.045),0,.95)
    ...
    cyan=material('cyan_accent_01',(.01,.6,.85),.25,.25); amber=material('amber_muted_01',(.64,.28,.055),.1,.4)
    ...
    if seeded: width=.168; height=.262
    cube('body_shell_01',(0,0,height/2+.012),(width,.154,height),chrome if seeded else ceramic,.028)
    cube('acoustic_grille_01',(0,-.078,height*.48+.014),(width*.86,.012,height*.78),textile,.023)
    cube('base_01',(0,0,.012),(width*.88,.132,.024),dark,.01)
    ...
    bpy.ops.object.text_add(location=(-.015,-.085,.021 if seeded else max(.043,float(rules.get('DETAIL-02',30))/1000)),rotation=(math.pi/2,0,0))
    logo=bpy.context.object; logo.name='logo_01'; logo['object_id']='logo_01'; logo.data.body='v e l a'; logo.data.size=.009; logo.data.extrude=.0001; logo.data.materials.append(ceramic)
